...
By offering hooks for the entire pipeline, we allow develoeprs to create plugins that add complete features to your GraphQL pipeline, without creating spaghetti code, or code that's specific your your application.
...
The core implementation of `envelop` is agnostic to the HTTP server you use, and agnostic to the way your define your GraphQL Schema, so it's not a traditional "server" framework (you can create plugins that integrate with your HTTP server - or with your schema framework, if needed)

You can use Envelop with any environment (Node.js or browser) and any GraphQL workflow (client/server, client-side execute, or server-to-server). 
...
So if you are working in a microservice environment, or in an enterprise environment with many servers - you can now share the entire base GraphQL setup in envelop plugins, and extend each of the servers based on your needs.
