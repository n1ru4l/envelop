...
With new major version comes breaking changes. This section will guide you through the process of migrating from `v2` to `v3`.
...
### 2. Remove `useAsyncSchema` plugin
...
### 3. Rename `useLazyLoadedSchema` to `useSchemaByContext`
...
### 5. Update options for `useMaskedErrors` plugin
...
### 8. Remove `onResolverCalled`

We were wrapping the `GraphQLSchema` object but with the new version we no longer want to depend on a specific implementation. So we decided to drop this and instead provide a new plugin that will let you hook into this phase.
...
PR: https://github.com/n1ru4l/envelop/pull/1500
