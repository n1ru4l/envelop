...
We were wrapping the `GraphQLSchema` object but with the new version we no longer want to depend on a specific implmentation. So we decided to drop this and instead provide a new plugin that will let you hook into this phase.
