...
##### 1. Remove `defualtSkipError` from `@envelop/sentry`

Since we no longer support `EnvelopError` we have removed the default in Sentry plugin to skip `EnvelopError`. We recmmend you provide custom `skipError` function to skip the errors you want.

```diff
- import { envelop } from '@envelop/core';
+ import { envelop, isGraphQLError } from '@envelop/core';
...
const getEnvelop = envelop({
  ...
  plugins: [
    useSentry({
      skipError: (error) => {
        // Skip any errors that are not `GraphQLError`
