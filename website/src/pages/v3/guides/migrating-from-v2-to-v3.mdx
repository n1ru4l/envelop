...
We have designed the new `envelop` to be engine agnostic. This means that `graphql` is no longer a peer dependency and `envelop` simply just wraps the `parse`, `validate`, `execute` and `subscribe` functions that you provide.
This allows you to use any GraphQL engine you want.
