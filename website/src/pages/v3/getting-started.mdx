...
```ts
import { parse, validate, specifiedRules, execute, subcribe } from 'graphql'
...
After starting this server we can send a GraphQL operation using `curl`, yielding a response of `{"data":{"__typename":"Query"}}{:json}`.
