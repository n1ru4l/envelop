...
Instruments don't have access to the input and outputs of a phase, use hooks to access that data.
...
If multiple plugins have `instruments`, they are composed in the same order as they are defined in the
plugin array (from top to bottom).
