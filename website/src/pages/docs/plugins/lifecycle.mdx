...
You can find the
[complete signature for plugins API here](https://github.com/n1ru4l/envelop/blob/d4c8f90f9fbccf3032b3b3b05f359b691076d25b/packages/types/src/index.ts#L55).
...
This `Instruments` instance allows wrapping an entire phase execution (including all plugin hooks);
meaning, running code just before, just after and around the execution of the phase.

Instruments don't have access to the input and outputs of a phase, use hooks to access that data. If
needed, we recommend to share data between instruments and hooks with a `WeakMap` and the given
`context` as the key.

All instruments take 2 parameters:
...
If multiple plugins have `instruments`, they are composed in the same order as they are defined in
the plugin array (from top to bottom).

It is possible to customize this composition if it doesn't suit your needs (ie. you need hooks and
instruments to have a different order of execution).
