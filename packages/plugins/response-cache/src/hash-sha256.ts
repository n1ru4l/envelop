...
if (typeof window === 'undefined' && !webCrypto) {
  ...
  try {
    ...
    nodeCrypto = Promise.resolve(new Function('require', 'return require("crypto")')(require));
  } catch (_error) {
    try {
      // eslint-disable-next-line no-new-func
...
export const hashSHA256 = async (text: string): Promise<string> => {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    console.warn(
      '[@envelop/response-cache]: The Node Crypto and Web Crypto APIs are not available.\n' +
