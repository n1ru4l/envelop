...
With the following execution result:
...
Subsequent execution of the operation with the same variables will be served from within the cache instead of going through the execution phase (and thus avoid doing round trips to databases or other network services). Resulting in much faster response times.

When a mutation is executed and the execution result contains an entity that is included within a cached execution result, the cache for that query operation is invalidated. Subsequent request will hit the execution phase.

```graphql
mutation RepositoryAddMutation($userId: ID, $repositoryName: String!) {
  repositoryAdd(userId: $userId, repositoryName: $repositoryName) {
    user {
      id # selecting the id causes the previous query operation to be invalidated
    }
  }
}
```
