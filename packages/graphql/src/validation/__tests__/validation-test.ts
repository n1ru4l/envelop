    ...
    function customRule(context: ValidationContext) {
      return {
        Directive(node: DirectiveNode) {
          ...
          const error = new GraphQLError('Reporting directive: ' + String(directiveDef), { nodes: node });
...
describe('Validate: Limit maximum number of validation errors', () => {
  ...
  it('passthrough exceptions from rules', () => {
    ...
    expect(() => validate(testSchema, doc, [customRule], { maxErrors: 1 })).toThrow(/^Error from custom rule!$/);
  });
});
