...
import type { GraphQLInterfaceType, GraphQLObjectType, GraphQLOutputType } from '../../type/definition';
    ...
    Field(node: FieldNode) {
      ...
      if (type) {
        ...
        if (!fieldDef) {
          ...
          context.reportError(
            new GraphQLError(`Cannot query field "${fieldName}" on type "${type.name}".` + suggestion, { nodes: node })
          );
        }
      }
    },
 ...
 */
function getSuggestedTypeNames(schema: GraphQLSchema, type: GraphQLOutputType, fieldName: string): Array<string> {
 ...
 */
function getSuggestedFieldNames(type: GraphQLOutputType, fieldName: string): Array<string> {
