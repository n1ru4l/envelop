  ...
  return {
    ...
    ObjectValue(node) {
      ...
      for (const fieldDef of Object.values(type.getFields())) {
        ...
        if (!fieldNode && isRequiredInputField(fieldDef)) {
          ...
          context.reportError(
            new GraphQLError(`Field "${type.name}.${fieldDef.name}" of required type "${typeStr}" was not provided.`, {
              nodes: node,
            })
          );
        }
    ...
    NullValue(node) {
      ...
      if (isNonNullType(type)) {
        context.reportError(
          new GraphQLError(`Expected value of type "${inspect(type)}", found ${print(node)}.`, { nodes: node })
        );
      }
    },
...
function isValidValueNode(context: ValidationContext, node: ValueNode): void {
  ...
  if (!isLeafType(type)) {
    ...
    context.reportError(
      new GraphQLError(`Expected value of type "${typeStr}", found ${print(node)}.`, { nodes: node })
    );
    return;
  }
