...
describe('Execute: Accepts any iterable as list value', () => {
  ...
  it('Accepts a Set as a List value', () => {
    ...
    expect(complete({ listField })).toEqual({
...
describe('Execute: Accepts async iterables as list value', () => {
  ...
  it('Handles promises from `completeValue` in AsyncIterables', async () => {
    expectJSON(await completeObjectList(({ index }) => Promise.resolve(index))).toDeepEqual({
  ...
  it('Handles nulls yielded by async generator', async () => {
    ...
    expect(await complete({ listField }, '[Int]')).toEqual({
...
describe('Execute: Handles list nullability', () => {
  async function complete(args: { listField: unknown; as: string }) {
    ...
    if (Array.isArray(listField)) {
      ...
      expectJSON(await executeQuery(promisify(listOfPromises))).toDeepEqual(result);
    }
    return result;
  ...
  it('Contains values', async () => {
    ...
    expect(await complete({ listField, as: '[Int]' })).toEqual({
      data: { listField: [1, 2] },
    });
    expect(await complete({ listField, as: '[Int]!' })).toEqual({
      data: { listField: [1, 2] },
