...
describe('Execute: Union and intersection types', () => {
  it('can introspect on union and intersection types', () => {
    ...
    expect(executeSync({ schema, document })).toEqual({
      data: {
        Named: {
          kind: 'INTERFACE',
          name: 'Named',
  ...
  it('executes using union types', () => {
    ...
    expect(executeSync({ schema, document, rootValue: john })).toEqual({
      data: {
        __typename: 'Person',
        name: 'John',
        pets: [
  ...
  it('gets execution info in resolver', () => {
    ...
    expect(encounteredRootValue).toEqual(rootValue);
    expect(encounteredContext).toEqual(contextValue);
  });
});
