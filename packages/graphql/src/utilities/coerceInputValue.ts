...
function defaultOnError(path: ReadonlyArray<string | number>, invalidValue: unknown, error: GraphQLError): void {
...
function coerceInputValueImpl(
  ...
  if (isNonNullType(type)) {
    ...
    onError(
      pathToArray(path),
      inputValue,
      new GraphQLError(`Expected non-nullable type "${inspect(type)}" not to be null.`)
    );
    return;
  }
      ...
      if (fieldValue === undefined) {
        if (field.defaultValue !== undefined) {
          ...
          onError(
            pathToArray(path),
            inputValue,
            new GraphQLError(`Field "${field.name}" of required type "${typeStr}" was not provided.`)
          );
        }
        continue;
      }
    ...
    for (const fieldName of Object.keys(inputValue)) {
      if (!fieldDefs[fieldName]) {
        ...
        onError(
          pathToArray(path),
          inputValue,
          new GraphQLError(`Field "${fieldName}" is not defined by type "${type.name}".` + didYouMean(suggestions))
        );
      }
    }
    return coercedValue;
