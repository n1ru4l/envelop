...
export function lexicographicSortSchema(schema: GraphQLSchema): GraphQLSchema {
  ...
  function replaceMaybeType<T extends GraphQLNamedType>(maybeType: Maybe<T>): Maybe<T> {
...
function sortObjMap<T, R>(map: ObjMap<T>, sortValueFn: (value: T) => R): ObjMap<R> {
...
function sortByName<T extends { readonly name: string }>(array: ReadonlyArray<T>): Array<T> {
  return sortBy(array, obj => obj.name);
}

function sortBy<T>(array: ReadonlyArray<T>, mapToKey: (item: T) => string): Array<T> {
