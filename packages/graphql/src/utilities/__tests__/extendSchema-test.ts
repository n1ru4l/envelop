...
function expectSchemaChanges(schema: GraphQLSchema, extendedSchema: GraphQLSchema) {
...
describe('extendSchema', () => {
  it('returns the original schema when there are no type definitions', () => {
    ...
    expect(extendedSchema).toEqual(schema);
  });
  ...
  it('Do not modify built-in types and directives', () => {
    ...
    // Built-ins are used
    expect(extendedSchema.getType('Int')).toEqual(GraphQLInt);
    expect(extendedSchema.getType('Float')).toEqual(GraphQLFloat);
    expect(extendedSchema.getType('String')).toEqual(GraphQLString);
  ...
  it('extends objects with standard type fields', () => {
    ...
    expect(validateSchema(extendedSchema)).toEqual([]);
    expect(extendedSchema.getType('Int')).toEqual(undefined);
    expect(extendedSchema.getType('Float')).toEqual(undefined);
    expect(extendedSchema.getType('String')).toEqual(GraphQLString);
    expect(extendedSchema.getType('Boolean')).toEqual(GraphQLBoolean);
    expect(extendedSchema.getType('ID')).toEqual(undefined);
    ...
    expect(extendedTwiceSchema.getType('Int')).toEqual(GraphQLInt);
    expect(extendedTwiceSchema.getType('Float')).toEqual(GraphQLFloat);
    expect(extendedTwiceSchema.getType('String')).toEqual(GraphQLString);
    expect(extendedTwiceSchema.getType('Boolean')).toEqual(GraphQLBoolean);
    expect(extendedTwiceSchema.getType('ID')).toEqual(GraphQLID);
  });
  ...
  it('extends scalars by adding new directives', () => {
    ...
    expect(validateSchema(extendedSchema)).toEqual([]);
    expectExtensionASTNodes(someScalar).toEqual(extensionSDL);
  });
  ...
  it('extends scalars by adding specifiedBy directive', () => {
    ...
    expect(foo.specifiedByURL).toEqual('https://example.com/foo_spec');

    expect(validateSchema(extendedSchema)).toEqual([]);
    expectExtensionASTNodes(foo).toEqual(extensionSDL);
  });
  ...
  it('correctly assign AST nodes to new and extended types', () => {
    ...
    expect(printSchema(extendedInOneGoSchema)).toEqual(printSchema(extendedTwiceSchema));
    ...
    expect(testType.extensionASTNodes).toEqual([]);
    expect(testEnum.extensionASTNodes).toEqual([]);
    expect(testUnion.extensionASTNodes).toEqual([]);
    expect(testInput.extensionASTNodes).toEqual([]);
    expect(testInterface.extensionASTNodes).toEqual([]);
  ...
  it('adds new unused types', () => {
    ...
    expect(validateSchema(extendedSchema)).toEqual([]);
    expectSchemaChanges(schema, extendedSchema).toEqual(extensionSDL);
  });
  ...
  it('extends different types multiple times', () => {
    ...
    expectSchemaChanges(schema, schemaWithNewTypes).toEqual(newTypesSDL);
  ...
  it('Rejects invalid SDL', () => {
    ...
    expect(() => extendSchema(schema, extendAST)).toThrow('Unknown directive "@unknown".');
  });
  ...
  it('Throws on unknown types', () => {
    ...
    `);
    expect(() => extendSchema(schema, ast, { assumeValidSDL: true })).toThrow('Unknown type: "UnknownType".');
  });
  ...
  it('does not allow replacing a default directive', () => {
    ...
    expect(() => extendSchema(schema, extendAST)).toThrow(
  ...
  describe('can add additional root operation types', () => {
    ...
    it('adds schema definition missing in the original schema', () => {
      const schema = buildSchema(`
        ...
        type Foo
      `);
      expect(schema.getQueryType()).toEqual(undefined);
    ...
    it('schema extension AST are available from schema object', () => {
      ...
      expectExtensionASTNodes(extendedTwiceSchema).toEqual(dedent`
        extend schema {
          mutation: Mutation
        }
