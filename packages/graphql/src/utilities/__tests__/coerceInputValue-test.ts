...
function coerceValue(inputValue: unknown, type: GraphQLInputType): CoerceResult {
  ...
  const value = coerceInputValue(inputValue, type, (path, invalidValue, error) => {
...
describe('coerceInputValue', () => {
  ...
  describe('for GraphQLEnum', () => {
    ...
    it('returns no error for a known enum name', () => {
      ...
      expectValue(fooResult).toEqual('InternalFoo');

      const barResult = coerceValue('BAR', TestEnum);
      expectValue(barResult).toEqual(123456789);
    });
  ...
  describe('with default onError', () => {
    it('throw error without path', () => {
      expect(() => coerceInputValue(null, new GraphQLNonNull(GraphQLInt))).toThrow(
        'Invalid value null: Expected non-nullable type "Int!" not to be null.'
      );
    });

    it('throw error with path', () => {
      expect(() => coerceInputValue([null], new GraphQLList(new GraphQLNonNull(GraphQLInt)))).toThrow(
