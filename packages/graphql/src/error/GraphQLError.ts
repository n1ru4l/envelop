...
function toNormalizedOptions(args: BackwardsCompatibleArgs): GraphQLErrorOptions {
...
export class GraphQLError extends Error {
  ...
  constructor(message: string, ...rawArgs: BackwardsCompatibleArgs) {
    ...
    const nodeLocations = undefinedIfEmpty(
      this.nodes?.map(node => node.loc).filter((loc): loc is Location => loc != null)
    );
    ...
    const originalExtensions = isObjectLike(originalError?.extensions) ? originalError?.extensions : undefined;
...
function undefinedIfEmpty<T>(array: Array<T> | undefined): Array<T> | undefined {
