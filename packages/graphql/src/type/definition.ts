...
export function resolveReadonlyArrayThunk<T>(thunk: ThunkReadonlyArray<T>): ReadonlyArray<T> {
...
export class GraphQLScalarType<TInternal = unknown, TExternal = TInternal> {
  ...
  constructor(config: Readonly<GraphQLScalarTypeConfig<TInternal, TExternal>>) {
    const parseValue = config.parseValue ?? (identityFunc as GraphQLScalarValueParser<TInternal>);

    this.name = assertName(config.name);
    this.description = config.description;
    this.specifiedByURL = config.specifiedByURL;
    this.serialize = config.serialize ?? (identityFunc as GraphQLScalarSerializer<TExternal>);
...
function defineInterfaces(
  config: Readonly<GraphQLObjectTypeConfig<any, any> | GraphQLInterfaceTypeConfig<any, any>>
...
function defineFieldMap<TSource, TContext>(
  config: Readonly<GraphQLObjectTypeConfig<TSource, TContext> | GraphQLInterfaceTypeConfig<TSource, TContext>>
...
export function defineArguments(config: GraphQLFieldConfigArgumentMap): ReadonlyArray<GraphQLArgument> {
 ...
 * @internal
 */
export function argsToArgsConfig(args: ReadonlyArray<GraphQLArgument>): GraphQLFieldConfigArgumentMap {
  return keyValMap(
    args,
    arg => arg.name,
    arg => ({
...
export type GraphQLFieldResolver<TSource, TContext, TArgs = any, TResult = unknown> = (
  source: TSource,
  args: TArgs,
  context: TContext,
  info: GraphQLResolveInfo
...
export interface GraphQLInterfaceTypeNormalizedConfig extends GraphQLInterfaceTypeConfig<any, any> {
  interfaces: ReadonlyArray<GraphQLInterfaceType>;
  fields: GraphQLFieldConfigMap<any, any>;
  extensions: Readonly<GraphQLInterfaceTypeExtensions>;
  extensionASTNodes: ReadonlyArray<InterfaceTypeExtensionNode>;
...
function defineTypes(config: Readonly<GraphQLUnionTypeConfig<unknown, unknown>>): ReadonlyArray<GraphQLObjectType> {
...
export class GraphQLEnumType /* <T> */ {
  ...
  parseLiteral(valueNode: ValueNode, _variables: Maybe<ObjMap<unknown>>): Maybe<any> /* T */ {
...
function defineInputFieldMap(config: Readonly<GraphQLInputObjectTypeConfig>): GraphQLInputFieldMap {
  const fieldMap = resolveObjMapThunk(config.fields);
  return mapValue(fieldMap, (fieldConfig, fieldName) => {
    devAssert(
