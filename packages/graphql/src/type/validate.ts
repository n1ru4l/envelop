...
class SchemaValidationContext {
  ...
  reportError(message: string, nodes?: ReadonlyArray<Maybe<ASTNode>> | Maybe<ASTNode>): void {
    const _nodes = Array.isArray(nodes) ? (nodes.filter(Boolean) as ReadonlyArray<ASTNode>) : (nodes as Maybe<ASTNode>);
...
function validateRootTypes(context: SchemaValidationContext): void {
  ...
  const rootTypesMap = new AccumulatorMap<GraphQLObjectType, OperationTypeNode>();
    ...
    if (rootType != null) {
      if (!isObjectType(rootType)) {
        ...
        context.reportError(
          ...
          getOperationTypeNode(schema, operationType) ?? (rootType as any).astNode
        );
      } else {
  ...
  for (const [rootType, operationTypes] of rootTypesMap.entries()) {
    if (operationTypes.length > 1) {
      ...
      context.reportError(
        ...
        operationTypes.map(operationType => getOperationTypeNode(schema, operationType))
      );
    }
  }
}

function getOperationTypeNode(schema: GraphQLSchema, operation: OperationTypeNode): Maybe<ASTNode> {
...
function validateDirectives(context: SchemaValidationContext): void {
  for (const directive of context.schema.getDirectives()) {
    ...
    if (!isDirective(directive)) {
      context.reportError(`Expected directive but got: ${inspect(directive)}.`, (directive as any)?.astNode);
      continue;
    }
...
function validateTypes(context: SchemaValidationContext): void {
  ...
  for (const type of Object.values(typeMap)) {
    ...
    if (!isNamedType(type)) {
      context.reportError(`Expected GraphQL named type but got: ${inspect(type)}.`, (type as any).astNode);
      continue;
    }
...
function validateFields(context: SchemaValidationContext, type: GraphQLObjectType | GraphQLInterfaceType): void {
...
function validateInterfaces(context: SchemaValidationContext, type: GraphQLObjectType | GraphQLInterfaceType): void {
  ...
  for (const transitive of iface.getInterfaces()) {
    if (!ifaceInterfaces.includes(transitive)) {
      context.reportError(
        ...
        [...getAllImplementsInterfaceNodes(iface, transitive), ...getAllImplementsInterfaceNodes(type, iface)]
      );
    }
  }
}

function validateUnionMembers(context: SchemaValidationContext, union: GraphQLUnionType): void {
...
function validateEnumValues(context: SchemaValidationContext, enumType: GraphQLEnumType): void {
...
function validateInputFields(context: SchemaValidationContext, inputObj: GraphQLInputObjectType): void {
...
function getUnionMemberTypeNodes(union: GraphQLUnionType, typeName: string): Maybe<ReadonlyArray<NamedTypeNode>> {
