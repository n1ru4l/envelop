...
function withModifiers<T extends GraphQLNamedType>(
  ...
  return [type, new GraphQLList(type), new GraphQLNonNull(type), new GraphQLNonNull(new GraphQLList(type))];
}
...
const notOutputTypes: ReadonlyArray<GraphQLInputType> = [...withModifiers(SomeInputObjectType)];
...
describe('Type System: A Schema must have Object root types', () => {
  ...
  it('rejects a schema extended with invalid root types', () => {
    ...
    expectJSON(validateSchema(schema)).toDeepEqual([
      ...
      {
        message: 'Mutation root type must be Object type if provided, it cannot be SomeScalar.',
        locations: [{ line: 3, column: 21 }],
      },
      {
        message: 'Subscription root type must be Object type if provided, it cannot be SomeEnum.',
...
describe('Type System: Root types must all be different if provided', () => {
  ...
  it('rejects a Schema where the same type is used for multiple root types', () => {
    ...
    expectJSON(validateSchema(schema)).toDeepEqual([
      {
        message: 'All root types must be different, "SomeObject" type is used as query and subscription root types.',
        locations: [
          { line: 11, column: 16 },
          { line: 13, column: 23 },
        ],
...
describe('Type System: Object fields must have output types', () => {
  function schemaWithObjectField(fieldConfig: GraphQLFieldConfig<unknown, unknown>): GraphQLSchema {
...
describe('Type System: Interface fields must have output types', () => {
  function schemaWithInterfaceField(fieldConfig: GraphQLFieldConfig<unknown, unknown>): GraphQLSchema {
...
describe('Type System: Input Object fields must have input types', () => {
  function schemaWithInputField(inputFieldConfig: GraphQLInputFieldConfig): GraphQLSchema {
...
describe('assertValidSchema', () => {
  ...
  it('combines multiple errors', () => {
    ...
    expect(() => assertValidSchema(schema)).toThrow(dedent`
