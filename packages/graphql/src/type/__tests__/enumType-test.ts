...
function executeQuery(source: string, variableValues?: { readonly [variable: string]: unknown }) {
...
describe('Type System: Enum Values', () => {
  ...
  it('presents a getValue() API for complex enums', () => {
    ...
    expect(badUsage).toEqual(undefined);
  });
  ...
  it('can be introspected without error', () => {
    expect(() => introspectionFromSchema(schema)).not.toThrow();
  });
});
